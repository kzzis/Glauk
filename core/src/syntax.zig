//! Line-based code-fence highlighting; block-comment state carries across lines.
...
const c_kw = [_][]const u8{
    ...
    "void",      "volatile", "while",   "final",    "import",    "package",  "super",    "var",
    "string",    "object",   "async",   "await",    "override",  "abstract", "readonly", "internal",
    "params",    "out",      "ref",     "is",       "as",        "in",       "foreach",  "lock",
    "yield",     "get",      "set",     "record",   "partial",   "sealed",   "base",     "null",
    "when",      "where",    "nameof",  "typeof",
};
...
pub fn langFromInfo(info_raw: []const u8) Lang {
    ...
    {
        return .{ .line_comment = "//", .block_open = "/*", .block_close = "*/", .quotes = "\"'`", .keywords = &js_kw };
    }
    ...
    if (eq(u8, lower, "go")) {
        return .{ .line_comment = "//", .block_open = "/*", .block_close = "*/", .quotes = "\"`", .keywords = &go_kw };
    }
    ...
    {
        return .{ .line_comment = "//", .block_open = "/*", .block_close = "*/", .quotes = "\"'", .keywords = &c_kw };
    }
...
/// Color diff lines as a whole; do not tokenize their embedded source code.
    ...
    while (i < line.len) {
        ...
        if (isIdentStart(c)) {
            ...
            const kind: ?TokenKind = if (isKeyword(lang, word))
                ...
                // Capitalized identifiers are treated as type names heuristically.
                .type_name
            else
                null;
            if (kind) |k| {
...
test "keywords, strings and numbers are picked up" {
    ...
    try testing.expectEqual(TokenKind.keyword, toks[0].kind);
    try testing.expectEqual(TokenKind.number, toks[1].kind);
    try testing.expectEqual(TokenKind.string, toks[2].kind);
}

test "uppercase identifiers are types and identifiers before ( are functions" {
    const gpa = testing.allocator;
    var in_block = false;
    const toks = try collect(gpa, "var b = WebApplication.CreateBuilder(args);", langFromInfo("cs"), &in_block);
    defer gpa.free(toks);

    try testing.expectEqual(TokenKind.keyword, toks[0].kind);
    try testing.expectEqual(TokenKind.type_name, toks[1].kind);
    try testing.expectEqual(TokenKind.function, toks[2].kind);
}
...
test "block comments carry across lines" {
    ...
    try testing.expectEqual(TokenKind.keyword, c[1].kind);
}
...
test "escaped quotes do not end the string early" {
    ...
    try testing.expectEqual(TokenKind.string, toks[0].kind);
    try testing.expectEqual(@as(usize, 6), toks[0].len);
